...
const ModulePage = () => {
  ...
  const handleDelete = () => {
    ...
    if (selectedRowData) {
      const updatedData = modulesData.filter(
        (module) => module.id !== selectedRowData.id
      );
      setModulesData(updatedData); // Update the data state
        ...
        return (
          <div className="flex space-x-2">
            <Button
              ...
              className=" h-7 w-7"
              color="secondary"
              onClick={() => handleEditClickBtn(row.original)}
            >
  ...
  return (
    <>
      ...
      <Dialog open={openEditModel} onOpenChange={(open) => {setOpenEditModel(open); }} >
        <DialogContent size="2xl">
          <DialogHeader>
            <DialogTitle className="text-base font-bold text-default-700 ">
